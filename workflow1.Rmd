...
In addition, the function requires to specify the class case and the combinations of interest. 
...
In this example 3 combinations have been chosen among the 14 contained in "rmks" object.
```{r}
reports <-ROC_reports(data, markers_table = mks, case_class = 'A', 
         selected_combinations = c(1,10,16), direction = '<')
```
...
To be more clear about the interpretation of results, this section will be focused on "Combination 16" (Marker1+Marker2+Marker3) which has an optimal cutoff equal to  0.216 (see **ROC curves**, reports['Metrics']) .
...
For "Combination 16", the predictions have been calculated in this way (see **ROC curves**, reports['Models']):


$$
f(x)=    -17.0128  +  1.5378 *log(Marker1 + 1)  +  0.9176 *log(Marker2 + 1) + 0.5706* log(Marker3 + 1)$$
Predictions (f(x) values) of 'Combination 16' can be visualized with the following command: 
```{r}
head(predict(reports$Models$`Combination  16`, type='link')) # link = f(x)
tail(predict(reports$Models$`Combination  16`, type='link'))  # link = f(x)
```


Prediction probabilities (p(x) values) of 'Combination 16' can be instead visualized by typing: 
```{r}
head(predict(reports$Models$`Combination  16`, type='response')) # response = p(x)
tail(predict(reports$Models$`Combination  16`, type='response')) # response = p(x)
```
...
Specifically, for "Combination 16":
...
Thus, using 0.216 as cutoff, "Combination 16" is able to classify the samples in the dataset with a SE equal to 95.0%, SP equal to 86.9%, and accuracy equal to 88.8% (see **ROC curves**, reports['Metrics']).
