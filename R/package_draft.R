...
# tutorial transcriptomics (DATA MODIFIED IN $CLASS VALUES AND MARKERS NAMES)

data <- load("data/demo_transcriptomics.csv", sep=',')# to load the data and check
# the data format

data_long <- CombiROC_long(data) # to make data in long format

markers_overview(data_long, ylim =60) # to plot boxplot with selecting
# the y value lim (zoom only, no data loss) and prints summary for each
#class

mks <-Combi(data, signalthr = 8, combithr = 1) # to compute combinations
# and count corresponding positive samples for each class (once thresholds
# are selected)
