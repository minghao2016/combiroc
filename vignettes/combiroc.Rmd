...
All these plotting functions take as input the mks dataframe obtained with the SE_SP() function and containing the SE and SP values for cases and controls in all marker combinations. The original mks dataframe can be customly filtered to plot only a subset of combinations meeting specific criteria.  
...
mks %>% 
  ggplot(aes(x = `SP%_B`, y = `SE%_A`, size = `#Markers` )) +
...
If the user wants to highlight the "Golden" combinations (how they were called in the CombiROC web-app), i.e. those above chosen thresholds of SE and SP, first the user need to set the custom cutoffs to the desired minimim values (here 40 and 80 for SE and SP respectively).
...
mks %>%
  mutate(Combo = ifelse((`SP%_B`>80 & `SE%_A`>40),"gold","below_thr")) %>% 
  ggplot(aes(x = `SP%_B`, y = `SE%_A`, size = `#Markers`, color = Combo)) +
