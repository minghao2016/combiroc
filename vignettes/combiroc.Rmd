...
As discussed before, `signalthr` should be set depending on the guidelines of the kit used for the analysis or by an accurate inspection of signal intensity distribution (see [Markers distribution overview](#distr) section). In this specific case 450 is the mean of the intensity of buffer + 3 standard deviation (see [Bombaci & Rossi 2019](https://link.springer.com/protocol/10.1007%2F978-1-4939-9164-8_16)).
...
## Bubble plot of the "Gold" combinations 
...
If the user wants to highlight the "Golden" combinations (how they were called in the CombiROC web-app), i.e. those above chosen thresholds of SE and SP, first the user need to set the custom cutoffs to the desired minimum values (here 40 and 80 for SE and SP respectively).

```{r, message=FALSE}
library(dplyr) # needed for the mutate() function and %>% operator
library(ggplot2)

min_SE <- 40 # arbitrarily choosen SE cutoff (as in the web-app slider)
min_SP <- 80 # arbitrarily choosen SP cutoff (as in the web-app slider)
