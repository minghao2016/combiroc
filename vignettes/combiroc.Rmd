...
The CombiROC package evolved from the [CombiROC Shiny Web app](http://combiroc.eu/) and it is an ongoing project. While the Shiny app is still working, we plan to mantain and improve the package with new functionalities. Here we offer tips on how to further exploring the data taking advantage into ; some of these features may be natively included in future versions of the CombiROC package.  

## Individual markers exploration

Further markers exploration can be done using other packages' functions such as `group_by()`, `summarize()` and `ggplot()` on the the `data_long` dataframe which contains all markers data in long format.  
As an example, a few summary statistics can be computed as follow (the libraries `dplyr` and `moments` are needed, the latter is used to calculate the Skewness)
