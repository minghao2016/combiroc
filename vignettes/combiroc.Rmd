---
title: "Getting started with CombiROC"
subtitle: |
...
output: rmarkdown::html_document 
  ...
  #   df_print: paged
vignette: >
  %\VignetteIndexEntry{Getting started with combiroc}
...
distribution$ROC
```
The __Coord__ is a dataframe that contains the coordinates of the above described "ROC" (threshold, SP and SE) that have at least a min SE (40 by default) and a min SP (80 by default). These limits can be set manually by specifying  min_SE and min_SP.
```{r, coord}
distribution$Coord
```
...
Finally, the __Density_summary__ displays summary statistics of the density plot:
```{r, density}
distribution$Density_summary
```

In case of lack of a priori known threshold the user can set set signalthr_prediction= TRUE. In this way the function provides a "suggested signal threshold" that corresponds to the median of the singnal threshold values (in "Coord") at which SE/SP are greater or equal to their set minimal values (min_SE and min_SP), and it adds this threshold on the "Density_plot" object as a dashed black line. The use of the median allows to pick a threshold whose SE/SP are not too close to the limits (min_SE and min_SP), but it is recommended to always inspect "Coord" and choose the most appropriate signal threshold by considering SP, SE and Youden index.
...
In this example have been chosen a single marker (Marker1) and 2 combinations (combinations number 11 and 15).
```{r}
