...
The __data__ object in the original wide format can be thus transformed into the reshaped long format __data_long__ object, and further used. See also the section ["Tips & tricks"](#tips) for some plotting examples taking advantage of the reshaped format.
...
Individual markers can also be explored plotting them one by one or retrieving summary statistics. In the section  ["Tips & tricks"](#tips) at the end of this vignette we suggest code snippets that can be used to [plot intensities of individual markers](#plot_int) across all samples, as well as to [calculate their summary statistics](#sum_stats). 
...
For a bit deeper discussion on how to interpret the results, this section will be focused on a single specific combination ("Combination 11"), combining Marker1, Marker2 and Marker3 in the dataset seen so far. This combination has an optimal cutoff equal to 0.216 (see  the __CutOff__ column in `reports$Metrics`).  
The following is the equation used to compute the predictions:
...
Where β_0 to β_n are the coefficients determined by the model.  
...
A new feature of the CombiROC package (not present in the CombiROC tool Shiny app), offers the possibility to exploit the models obtained with `roc_reports()` for each selected marker/combination (and assigned to `reports$Models`) to directly classify new samples that are not labelled, _i.e._ not assigned to any case or control class.

The unclassified dataset must be similar to the dataset used for the previous combinatorial analysis ( _i.e._ of the same nature and with the same markers, but obviously _without_ the 'Class' column).
...
For purely dimonstrative purposes, in the following example a "synthetic" unclassified dataset ('data/unclassified_proteomic_data.csv') was used: it was obtained by randomly picking 20 samples from the already classified dataset (the __data__). The loaded unclassified sample is here assinged to the __unc_data__ object.  
Please note that this dataset has a Patient.ID column which actually allows the identification of the class (a pre-pended "no" tag identifies healthy subjects) __but this column is not used in the workflow and has solely labeling purposes__.
```{r, eval=FALSE}
...
As can be observed comparing the outcome in the dataframe with the tag on samples' names, while the single marker __Marker1__ is not 100% efficient in correctly predicting the class (see second row), both markers combinations are.
...
# Additional tips&tricks and code snippets {#tips}
...
## Bubble plot of the all combinations {#bubble}
